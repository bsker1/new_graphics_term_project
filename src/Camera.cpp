...
Camera::Camera(const int inWidth, const int inHeight, const glm::vec3 inPostition) {
  width = inWidth;
  height = inHeight;
  position = inPostition;
}

void Camera::UpdateMatrix(const float yFovDegree, const float aspect, const float nearPlane, const float farPlane) {
  ...
  proj = glm::perspective(glm::radians(yFovDegree), aspect, nearPlane, farPlane);
...
void Camera::Inputs(GLFWwindow* window) {
  ...
  if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS) {
    ...
    if (!((glm::angle(newOrientation, upDirection) <= glm::radians(5.0f)) or (glm::angle(newOrientation, -upDirection) <= glm::radians(5.0f)))) {
